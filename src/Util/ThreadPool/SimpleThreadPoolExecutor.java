 ...
 *
 * This executor is trying to execute commands immediately using up to
 * {@link #setMaxPoolSize(int) maxPoolSize} threads.<br>
 * If the pool cannot be further extended then the commands are queued up to
 * {@link #setMaxQueuedTasks(int) maxQueuedTasks}.<br>
 * If even the queue is full then the execute methods will block the calling
 * thread.<br><br>
 *
 * <b>Note</b> that there is no explicit start method since the executor is
 * started with the first command to execute.<br>
...
public class SimpleThreadPoolExecutor extends AbstractExecutorService implements SimpleExecutorService {
    ...
    private final Deque<PoolThread> threadQueue = new LinkedList<>();
    ...
    public void setThreadGroup(ThreadGroup threadGroup) {
        if (threadGroup == null) {
            throw new IllegalArgumentException("Thread group required");
        }
    ...
    /**
     * Sets the pool thread base name, defaults to name of class
     * {@link PoolThread}.<br>
    ...
    public void setThreadName(String threadName) {
        if (threadName == null) {
            throw new IllegalArgumentException("Thread name required");
        }
    ...
    /**
     * Set to
     * <code>true</code> if pool threads should be
     * {@link Thread#setDaemon(boolean) daemon threads}, defaults to
     * <code>false</code>.
     */
    ...
    /**
     * Sets the {@link Thread#setPriority(int) thread priority}, defaults to
    ...
    /**
     * Sets the core pool size, defaults to
     * <code>0</code>.<br>
     * Once pool threads reach this level, it will be retained (core threads do
     * not {@link #setThreadTimeout(long, TimeUnit) time out}).
     *
    ...
    /**
     * Sets the maximum pool size, defaults to
     * <code>10</code>.<br>
     * Pool threads can {@link #setThreadTimeout(long, TimeUnit) time out} until
     * the {@link #setCorePoolSize(int) core level} is reached.
     *
    ...
    /**
     * Sets the pool thread timeout, defaults to
     * <code>60000 ms</code>.<br>
     * Only if thread level is higher than the
     * {@link #setCorePoolSize(int) core level} then idle threads will stop.
     *
    ...
    public void setThreadTimeout(long timeout, TimeUnit unit) {
        if (unit == null) {
            throw new IllegalArgumentException("Timeout unit required");
        }
    ...
    /**
     * Sets the limit of queued commands, defaults to
    ...
    public void setMaxQueuedTasks(int maxQueuedTasks) {
        if (maxQueuedTasks < 0) {
            throw new IllegalArgumentException("Invalid max queued tasks value");
        }
    ...
    public void shutdown() {
        ...
        try {
            ...
            if (totalThreads > 0) {
                ...
                System.out.println("Remaining threads: " + totalThreads + "remaining queued tasks: " + taskQueue.size());
    ...
    public List<Runnable> shutdownNow() {
        ...
        try {
            ...
            System.out.println("Shut down now, drained threads: " + remainingTasks.size() + "remaining threads: " + totalThreads);
    ...
    private boolean setState(int state) {
        for (;;) {
            ...
            if (this.state.compareAndSet(currentState, currentState | state)) {
                return true;
            }
        }
    }
    ...
    /**
     * Pre-start pool threads up to the
     * {@link #setCorePoolSize(int) core level}.
     */
    ...
    protected class PoolThread extends Thread {
        ...
        public PoolThread(Runnable command, ThreadGroup group, String name) {
            super(group, name);
            this.command = new AtomicReference<>(command);
        }
        ...
        /**
         * In a loop executes the set command and waits for new command,
         * optionally timing out.
         */
        @Override
        public void run() {
        ...
        /**
         * Tries to get next command, otherwise putting itself into the idle
         * thread pool.
         *
